...
// ── Bottom Tab Bar ──────────────────────────────────────────────────────────
const TABS = [
  { label: "Home", path: "/robo-live", icon: HomeIcon },
  { label: "Market", path: "/robo-live/market", icon: MarketIcon },
  { label: "Wallet", path: "/robo-live/wallet", icon: WalletIcon },
  { label: "Insights", path: "/robo-live/insights", icon: InsightsIcon },
  { label: "Profile", path: "/robo-live/login", icon: ProfileIcon },
];

export function RoboLiveTabBar() {
  ...
  return (
    ...
    >
      {TABS.map(({ label, path, icon: Icon }) => {
        const active = current === path || (path !== "/robo-live" && current.startsWith(path));
        return (
          <button
            key={path}
...
// ── Main Layout ────────────────────────────────────────────────────────────
interface RoboLiveLayoutProps {
  title?: string;
  children: React.ReactNode;
  hideTabBar?: boolean;
}

export default function RoboLiveLayout({ title = "E-Robo Wallet", children, hideTabBar = false }: RoboLiveLayoutProps) {
  return (
    <>
      <Head>
        <title>{title}</title>
...
// ── Coin Icon Helper ───────────────────────────────────────────────────────
export function CoinIcon({ src, size = 50 }: { src: string; size?: number }) {
  return (
    <div style={{ width: size, height: size, position: "relative", flexShrink: 0 }}>
      <img src={src} alt="" style={{ width: size, height: size, objectFit: "cover", borderRadius: "50%" }} />
    </div>
  );
}

// ── Change Badge ───────────────────────────────────────────────────────────
export function ChangeBadge({ value, up }: { value: string; up: boolean }) {
  return (
    <div className="flex items-center gap-0.5">
      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
        {up ? (
          ...
          <path d="M5 8L2 3H8L5 8Z" fill={L.RED} />
        )}
      </svg>
      <span style={{ fontSize: 13, fontWeight: 500, color: up ? L.GREEN : L.RED }}>{value}</span>
    </div>
  );
}
